...
from AB3DMOT_libs.utils_voe import Args, Dynamic, ObsModel
from AB3DMOT_libs.modeling.lib import RecurrentEncoder, GRU_Net
from AB3DMOT_libs.data.dataset import Batch
...
def create_forward_mlp(
    ...
    def forward(batch: Batch, is_training: bool=True):
        ...
        state = jnp.hstack((state,jnp.zeros((len(state), 3))))
        ...
        # negative_logprob_all = -obs_model(state_seq, obs_seq)
        # kl_all = kl_gaussians(mu_seq, logvar_seq, mu_prior, prior_logvar)
        # mask_kl = jnp.array(mu_seq<99)
        # kl_loss = (jnp.multiply(kl_all,mask_kl[:,:,0])).mean()
        # mask_logprob = jnp.array(state_seq<99)
        # negative_logprob = (jnp.multiply(negative_logprob_all,mask_logprob[:,0])).mean()

        kl_loss = kl_gaussians(mu_seq, logvar_seq, mu_prior, prior_logvar).mean()
        negative_logprob = -obs_model(state_seq, obs_seq).mean()
        
        total_loss =  beta_kl * kl_loss +  beta * negative_logprob
        # print(total_loss)
        metrics = {
            "loss/total": total_loss,
            "loss/kl": kl_loss,
        ...
        if not is_training:
            state_for_test = state_for_test[:,:,0:7]
