...
from AB3DMOT_libs.ukf_ctrv import UKF
from AB3DMOT_libs.aukf_ctrv import AUKF
from AB3DMOT_libs.ekf_ctrv import EKF
from AB3DMOT_libs.iekf import IEKF
from AB3DMOT_libs.huberukf_ctrv import HuberUKF
from AB3DMOT_libs.conv_ukf_ctrv import ConvUKF
from AB3DMOT_libs.a_conv_ukf_ctrv import AConvUKF
from AB3DMOT_libs.rekf import REKF
from AB3DMOT_libs.ickf import ICKF
from AB3DMOT_libs.voe_filter import Voe
from AB3DMOT_libs.plf import Plf
...
filter_type = HuberUKF  #  KF  Ekf  Ukf  Voe ConvKF
pc = 0.00
kain_c = 1000
print_time = False
...
class AB3DMOT(object):
    ...
    def print_param(self):
        ...
        print_log(
            "ego motion compensation is %d" % self.ego_com, log=self.log, display=False
        )
    ...
    def ego_motion_compensation(self, frame, trks):
        ...
        ego_xyz_imu, ego_rot_imu, left, right = get_ego_traj(
            self.oxts, frame, 1, 1, only_fut=True, inverse=True
        )
        for index in range(len(self.trackers)):
            trk_tmp = trks[index]
            xyz = np.array([trk_tmp.x, trk_tmp.y, trk_tmp.z]).reshape((1, -1))
            compensated = egomotion_compensation_ID(
                xyz, self.calib, ego_rot_imu, ego_xyz_imu, left, right
            )
    ...
    def prediction(self):
        ...
        # Initialize a variable to accumulate time differences in milliseconds.
        total_time_ms = 0
        num_iterations = 0
        ...
        for t in range(len(self.trackers)):
            ...
            # Accumulate the execution time of this iteration in milliseconds.
        ...
        # Calculate and print the average execution time per iteration in milliseconds.
        if (num_iterations > 0) & print_time:
            average_time_ms = total_time_ms / num_iterations
            print(f"predict time: {average_time_ms} ms, {num_iterations}iters")

        return trks

    def update(self, matched, unmatched_trks, dets, info):
        # update matched trackers with assigned detections
        # Initialize a variable to accumulate time differences.
        total_time = 0
        num_iterations = 0
        ...
        for t, trk in enumerate(self.trackers):
            if t not in unmatched_trks:
                ...
                # pc=0.05 mask
                probility = np.random.uniform(0, 1)
                if probility < pc:
                    bbox3d = bbox3d * kain_c
                    # np.random.seed(42)
                    # noise = np.random.normal(0, R, bbox3d.shape)
                    # bbox3d = bbox3d + noise

                trk.kf.x[3], bbox3d[3] = self.orientation_correction(
                    trk.kf.x[3], bbox3d[3]
                )
                # print(trk.kf.sigmas_f )
                # trk.kf.sigams_f = trk.kf.points_fn.sigma_points(trk.kf.x, trk.kf.P)
                # print(trk.kf.sigams_f)
                # print('trk.id:',trk.id)
                ...
                if trk.id == self.debug_id:
                    ...
                    # print(trk.kf.sigmas_f[0])
                    # trk.kf.sigams_f = trk.kf.points_fn.sigma_points(trk.kf.x, trk.kf.P)
                    # print(trk.kf.sigams_f[0])
                ...
                # Start timing before the code segment of interest.
                start_time = time.time()
                if (
                    (isinstance(trk, KF))
                    | (isinstance(trk, EKF))
                ...
                # elif (isinstance(trk, EKF)):
                # 	trk.kf.update(bbox3d, trk.kf.H, trk.kf.hx)
                elif isinstance(trk, Voe):
                    trk.kf.update(bbox3d, x_com=trk.kf.x)
                # elif (isinstance(trk, Plf)):
                # 	trk.kf.update(bbox3d, x=trk.kf.x)
                else:
                    trk.kf.update(bbox3d, x=trk.kf.x)

                # Stop timing after the code segment.
                end_time = time.time()

                # Accumulate the execution time of this iteration.
        ...
        # Calculate and print the average execution time per iteration.
        if (num_iterations > 0) & print_time:
            average_time = total_time / num_iterations
            print(f"update time: {average_time} ms, {num_iterations}iters")
    ...
    def birth(self, dets, info, unmatched_dets):
        ...
        for i in unmatched_dets:  # a scalar of index
            trk = filter_type(Box3D.bbox2array(dets[i]), info[i, :], self.ID_count[0])
