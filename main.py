...
def main_per_cat(cfg, cat, log, ID_start):
    ...
    for seq_name in seq_eval:
        ...
        tracker, frame_list = initialize(
            cfg, trk_root, save_dir, subfolder, seq_name, cat, ID_start, hw, log
        )

        # loop over frame
        print(int(frame_list[0]))
        print(int(frame_list[-1]))
        ...
        for frame in range(min_frame, max_frame + 1):
            ...
            # logging
            print_str = "processing %s %s: %d/%d, %d/%d   \r" % (
                result_sha,
                seq_name,
                seq_count,
                len(seq_eval),
...
def main(args):
    ...
    if args.det_name is not "":
        cfg.det_name = args.det_name

    # print configs
    time_str = get_timestring()
    log = os.path.join(
        cfg.save_root, "log/log_%s_%s_%s.txt" % (time_str, cfg.dataset, cfg.split)
    )
    ...
    combine_trk_cat(
        cfg.split, cfg.dataset, cfg.det_name, "H%d" % cfg.num_hypo, cfg.num_hypo
    )
    print_log("\nDone!", log=log)
    log.close()
